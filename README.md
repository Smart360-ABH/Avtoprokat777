...
3. **Настройте переменные окружения**:
   ...
   - Для запуска фронтенда отдельно (vite dev) и вызова локального бэкенда, укажите базовый URL API:
     - `VITE_API_URL=http://localhost:3000`
     - Когда `VITE_API_URL` задан, фронтенд будет делать запросы к этому хосту. Если не задан, он будет использовать относительные пути (`/api/...`).
4. **Соберите проект**:
   ```powershell
   pnpm run build
   ```
5. **Запустите проект**:
   ...
   pnpm run start
   ```
   - Для разработки фронтенда отдельно используйте:
     ```powershell
     pnpm --filter @workspace/autopro777 dev
     ```
